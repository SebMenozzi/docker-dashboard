...
const cmd_promise = (command) => new Promise((resolve, reject) => {
...
app.get('/api/containers', async (req, res) => {
    try {
        const containers = await cmd_promise('docker ps -a --format "{{.ID}}\t{{.Image}}\t{{.Status}}\t{{.Ports}}\t{{.Names}}"')
        ...
        for (let container of containers_list) {
            ...
            container_json['is_active'] = status ? status.split(' ')[0] == 'Up' : false
            container_json['port'] = port ? port.split(':')[1].split('-')[0] : ''
...
app.get('/api/instances', async (req, res) => {
    try {
        const instances = await cmd_promise('gcloud beta run services list --platform managed --format=json')
        ...
        for (let instance of instances_list) {
            result.push({
                ...
                'region': instance['metadata']['labels']['cloud.googleapis.com/location'],
                'max_scale': instance['spec']['template']['metadata']['annotations']['autoscaling.knative.dev/maxScale'],
                'image': instance['spec']['template']['metadata']['annotations']['client.knative.dev/user-image'],
                ...
                'concurrency': instance['spec']['template']['spec']['containerConcurrency'],
                'last_build': instance['spec']['template']['metadata']['name'],
                'timeout': instance['spec']['template']['spec']['timeoutSeconds'],
                'port': instance['spec']['template']['spec']['containers'][0]['ports'][0]['containerPort'],
                'cpu': instance['spec']['template']['spec']['containers'][0]['resources']['limits']['cpu'],
                'memory': instance['spec']['template']['spec']['containers'][0]['resources']['limits']['memory']
            })
        }
...
io.sockets.on('connection', (socket) =>  {
    ...
    socket.on('containers.stats', () => {
        statsProcesses[socket.id] = spawn('docker', [ 'stats', '--all', '--format', '{{.Name}}\t{{.CPUPerc}}\t{{.MemUsage}}' ])
        ...
        statsProcesses[socket.id].stdout.on('data', data => {
            ...
            if (stats.length > 0)
                io.emit('containers.stats', stats)
        })
    })

    socket.on('containers.stats.bye', () => {
        if (socket.id in statsProcesses)
            statsProcesses[socket.id].kill()
    })

    // get logs
    socket.on('containers.logs', (name) => {
        if (!(socket.id in logsProcesses)) {
            logsProcesses[socket.id] = spawn('docker', [ 'logs', name, '--tail', '200', '--follow' ])
