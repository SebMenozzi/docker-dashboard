        ...
        <div>
            {
                !isLoading &&
                (logs.length > 0) &&
                <Paper ref={el => (logsContainer.current = el)} sx={{
                    p: 2,
                    display: 'flex',
                    ...
                    <Typography component="h6" color="white" sx={{ padding: '10px' }}>
                    {
                        logs.map(log => {
                            return log.split('\n').map((item, i) => {
                                return <div dangerouslySetInnerHTML={{__html: convert.toHtml(item)}}></div>
                            })
                        })
                    }
                    </Typography>
            ...
            }
            {
                ...
                <NoResultComponent message={'No logs to show'} isLoading={isLoading} />
            }
        </div>
    )
